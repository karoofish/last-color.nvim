...
local write_cache_file = function(colorscheme, background)
  local fd = assert(open_cache_file('w'))
  assert(uv.fs_write(fd, string.format('%s-%s\n', colorscheme, background), -1))
  assert(uv.fs_close(fd))
end
...
M.setup = function()
  vim.api.nvim_create_autocmd('ColorScheme', {
    ...
    callback = function(info)
      ...
      local ok, result = pcall(write_cache_file, new_scheme, background)
      if not ok then
